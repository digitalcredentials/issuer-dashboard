...
export async function createCredential(prevState: State, formData: FormData) {
 ...
 // TODO: want to directly deal with 404's using notFound()
  try {
    ...
    const result = callStore('credential', 'POST', {holder_email: email, holder_name: holder, cred_name: credName, added_by: userName})
...
export async function updateCredential(
  ...
  try {
     const result = callStore(`credential/${id}`, 'PUT', {holder_email: email, holder_name: holder, cred_name: credName})
  } catch (error) {
