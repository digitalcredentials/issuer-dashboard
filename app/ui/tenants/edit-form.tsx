...
import { updateTenant, State } from '@/app/lib/tenants/actions';
...
export default function EditTenantForm({
  ...
  const initialState: State = { message: null, errors: {}, formData: {
    ...
    email: tenant.email,
    issuer_image_url: tenant.issuer_image_url,
    issuer_url: tenant.issuer_url,
    issuer_name: tenant.issuer_name,
    env_name: tenant.env_name,
    status: tenant.is_active ? 'active' :'deactivated'    } 
  };
  const updateTenantWithId = updateTenant.bind(null, tenant.id as string);
  const [state, formAction] = useActionState(updateTenantWithId, initialState);
